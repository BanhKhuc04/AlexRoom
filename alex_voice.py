...
from alex_brain_tools import BrainChatRequest, MAX_REQUEST_ID_LENGTH, BOUNDED_ID_PATTERN
...
def process_voice_transcript(
    ...
    try:
        ...
        has_mutations = len(response.tool_results) > 0 and any(
            getattr(t, "tool_call", None) and t.tool_call.name not in ("system_status", "list_devices")
        ...
        error_code = VoiceErrorCode.BRAIN_TIMEOUT if e.code == "brain_timeout" else VoiceErrorCode.BRAIN_UNAVAILABLE
        return VoiceResponse(
            session_id=voice_input.session_id,
            request_id=voice_input.request_id,
            assistant_text=None,
