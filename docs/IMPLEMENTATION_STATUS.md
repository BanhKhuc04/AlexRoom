...
- Backend FastAPI/MQTT: local broker hoạt động; giao tiếp cơ bản ESP01/test LED đã được xác nhận sau gate ban đầu. Toàn node chưa hardware-verified và không gửi relay command.
...
- Bằng chứng Phase 2 tại thời điểm chụp từng ghi ESP01 `unknown`; trạng thái hiện tại đã tiến tới basic physical validation cho ESP01/test LED. Không có relay command nào được gửi và không có tải nguy hiểm nào được xác minh.
...
- API v1 additive, giữ nguyên route v0.3. Snapshot tách connectivity, `reported_state`/`desired_state` và verification truth lấy từ `CapabilityRegistry`; không dùng một cờ global hard-code.

## Step 1.3 — Hardware truth registry & status consistency

- `CapabilityRegistry` là nguồn duy nhất cho verification status của ESP01 và từng capability.
- `esp01`: `basic_physical_validated`, nhưng `hardware_verified=false`; Phase 7 đầy đủ vẫn đang tiếp tục.
- `test_led`: `basic_physical_validated`, safe và available.
- `relay_1..4`: `restricted`, chưa hardware-verified và command bị từ chối trước transport.
- Presence, Spatial Home, Device workspace, status/device/command API đều hiển thị truth do server cung cấp; ONLINE không đồng nghĩa hardware verified.
- Safety denial audit lưu có cấu trúc node, capability, action, status, risk và reason trong SQLite schema v3.
