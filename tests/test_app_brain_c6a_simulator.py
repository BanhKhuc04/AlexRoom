...
from alex_orchestration import MissionExecutor  # noqa: E402
...
class CoreBrainC6ASimulatorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.store = AlexStore(Path(self.temp.name) / "c6a.db")
        self.store.migrate()
        ...
        self.store.put_record(
            ...
            "unsafe-relay-mission",
            {
                "name": "Local unsafe relay mission",
                "brain_allowed": True,
                "source": "simulated",
                "steps": [
                    {
    ...
    def test_safe_stored_mission_uses_real_executor_gateway_and_simulator(self) -> None:
        with (
            patch.object(
                alex_app,
        ...
        self.assertIn("mission_preflight", stages)
        self.assertIn("mission_execution_start", stages)
        self.assertIn("mission_execution_outcome", stages)

    def test_relay_mission_is_rejected_before_step_one_with_zero_effects(self) -> None:
    ...
    def test_production_app_preserves_c6a_mission_wiring(self) -> None:
        self.assertEqual(
            tuple(
                tool
                for tool in alex_app.core_brain_integration._execution_allowlist
                if tool in C6A_EXECUTION_ALLOWLIST
            ),
            C6A_EXECUTION_ALLOWLIST,
        )
        adapter = alex_app.core_brain_integration._safe_mission_executor
        self.assertIsInstance(adapter, StoredSafeMissionExecutor)
        self.assertIs(adapter._mission_executor, alex_app.mission_executor)
